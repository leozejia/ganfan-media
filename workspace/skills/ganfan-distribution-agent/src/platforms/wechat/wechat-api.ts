...
import { getWechatCommentFlags, loadWechatCredentialEnv, loadWechatProfile } from "./nexus-profile.ts";
...
function loadConfig(): WechatConfig {
  const localEnv = loadWechatCredentialEnv();
  const appId = process.env.WECHAT_APP_ID || localEnv.WECHAT_APP_ID;
  const appSecret = process.env.WECHAT_APP_SECRET || localEnv.WECHAT_APP_SECRET;

  if (!appId || !appSecret) {
    throw new Error(
      "Missing WECHAT_APP_ID or WECHAT_APP_SECRET.\n" +
      "Set via environment variables or in mantle/media_substrate/publishing/profiles/wechat.local.env."
    );
  }
...
async function publishToDraft(
  ...
  const { needOpenComment, onlyFansCanComment } = getWechatCommentFlags();
...
function renderMarkdownToHtml(markdownPath: string, theme: string = "magazine", color?: string, citeStatus: boolean = true): string {
  ...
  const htmlPath = resolveRenderedHtmlPath(markdownPath);

  const renderArgs = [renderScript, markdownPath, "--theme", theme, "--output", htmlPath];
...
Options:
  ...
  --theme <name>      Theme name for markdown (default, grace, simple, modern, magazine). Default: wechat profile
  --color <hex>        Primary color override (default from wechat profile)
...
Comments:
  Comment switches are read from mantle/media_substrate/publishing/profiles/wechat.yaml.
...
Config File Locations (in priority order):
  ...
  2. NEXUS_WECHAT_ENV_PATH
  3. mantle/media_substrate/publishing/profiles/wechat.local.env
...
Example:
  ...
  bun wechat-api.ts article.html --title "My Article"
  bun wechat-api.ts images/ --type newspic --title "Photo Album"
  bun wechat-api.ts article.md --dry-run
`);
  process.exit(0);
}
...
function parseArgs(argv: string[]): CliArgs {
  ...
  const args: CliArgs = {
    ...
    isHtml: false,
    articleType: "news",
    theme: profile.defaults.theme || "magazine",
    color: profile.defaults.color || '#FF8C38',
    citeStatus: true,
    dryRun: false,
  };
...
async function main(): Promise<void> {
  ...
  if (!author) {
    author = profile.defaults.author || "";
  }
