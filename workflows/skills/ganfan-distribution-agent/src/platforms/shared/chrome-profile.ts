...
function getRealUserHome(): string {
  ...
  const candidates = [
    process.env.NEXUS_REAL_HOME?.trim(),
...
export function resolveSharedAutomationChromeProfileDir(configuredPath?: string): string {
  const envOverride = process.env.NEXUS_SHARED_CHROME_PROFILE_PATH?.trim();
  if (envOverride) return envOverride;

  const configured = configuredPath?.trim();
  if (configured) return configured;

  return path.join(resolveRepoRoot(), '.runtime', 'openclaw', 'browser-profile');
}

export function resolveSharedAutomationChromeDebugPort(preferredPort?: number | null): number {
  const envPort = parsePort(process.env.NEXUS_SHARED_CHROME_DEBUG_PORT);
