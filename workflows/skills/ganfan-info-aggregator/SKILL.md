...
version: 1.0.0
description: GanFan Stage 1 信息聚合技能。当工作流进入信息收集阶段时必须使用。调用 agent-browser 做来源检索，输出可溯源的纯客观事实信息包；严禁输出角度建议、选题建议和倾向性结论。
author: GanFan Nexus
dependencies:
  - agent-browser
---
...
- 仅负责 Stage 1 信息收集。
- 输出是事实信息包，不是选题提案。
- 所有结论性动作留给后续 Stage 2/2.5。

## 输入 / 输出合同

- **输入**: `topic`、关键词、探索目标、项目路径。
- **输出**: `01-信息源/direction_proposal.md`（事实信息包）。
...
5. 写入 `01-信息源/direction_proposal.md`。
...
- 文件存在：`01-信息源/direction_proposal.md`
- 结构符合模板
- 关键事实全部可回溯
- 不含建议型内容
