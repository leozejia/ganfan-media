...
import { loadWechatProfile } from './channel-profile.ts';
...
export async function convertMarkdown(markdownPath: string, options?: { title?: string; theme?: string; color?: string; citeStatus?: boolean }): Promise<ParsedResult> {
  const baseDir = path.dirname(markdownPath);
  const content = fs.readFileSync(markdownPath, 'utf-8');
  const profile = loadWechatProfile();
  const theme = options?.theme ?? profile.defaults.theme ?? 'default';
  const color = options?.color ?? profile.defaults.color;
  ...
  const author = frontmatter.author || profile.defaults.author || '';
  ...
  const renderArgs = [renderScript, tempMdPath, '--theme', theme, '--keep-title'];
  if (color) renderArgs.push('--color', color);
  if (citeStatus) renderArgs.push('--cite');

  console.error(`[md-to-wechat] Rendering markdown with theme: ${theme}${color ? `, color: ${color}` : ''}, citeStatus: ${citeStatus}`);
...
Usage:
  bun md-to-wechat.ts <markdown_file> [options]

Options:
  --title <title>     Override title
  --theme <name>      Theme name (default, grace, simple, modern, magazine)
  --color <name|hex>  Primary color (or hex, default from wechat profile)
...
Example:
  bun md-to-wechat.ts article.md
  bun md-to-wechat.ts article.md --theme grace
  bun md-to-wechat.ts article.md --theme grace --color "#FF8C38"
  bun md-to-wechat.ts article.md --no-cite
`);
  process.exit(0);
}
